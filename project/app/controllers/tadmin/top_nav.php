...
class Top_nav extends Controller
{
	...
	{
		$data = array(
			'query'		=> $this->common_model->get_records('SELECT id,title,hplink,target,seqorder FROM '.$this->db->dbprefix.'nav WHERE is_top = 1 ORDER BY seqorder DESC,id DESC')
		);
	...
	function add_record_view()
	{
		...
		$this->load->view(TPL_FOLDER."top_nav_add");
	}
	...
	function edit_record()
	{
		...
		$this->load->config('sort_type');
		$this->load->view(TPL_FOLDER."top_nav_edit",$data);
	}
	...
	function save_record()
	{
		...
		$this->_write_cache();
		echo_msg('<li>记录修改成功.</li>',my_site_url(CTL_FOLDER.'top_nav'),'yes');
	}
	...
	{
		$query = $this->common_model->get_records('SELECT title,hplink,target FROM '.$this->db->dbprefix.'nav WHERE is_top = 1 ORDER BY seqorder DESC,id DESC');
		save_cache('top_nav',$query);
		unset($query);
	}
}
?>